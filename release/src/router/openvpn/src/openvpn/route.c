                ...
                struct env_set *es)
{
    ...
    if (rl->rgi.flags & RGI_IFACE_DEFINED)
    {
        setenv_str(es, "net_iface", rl->rgi.iface);
    }
#endif
    if (rl->rgi.flags & RGI_ADDR_DEFINED)
    {
...
#ifdef ASUSWRT
    setenv_unsigned(es, "rgi6_flags", rl6->rgi6.flags);
    if (rl6->rgi6.flags & RGI_IFACE_DEFINED)
    {
        setenv_str(es, "net_iface_ipv6", rl6->rgi6.iface);
    }
#endif
