...
#elif defined(RTCONFIG_WLMODULE_MT7663E_AP) || defined(RTCONFIG_WLMODULE_MT7629_AP) || defined(RTCONFIG_WLMODULE_MT7915D_AP)
	if (unit == 1 || unit == 0)
	{
		char *p = tmp;
		size_t len = sizeof(tmp);
...
#define WL_LOG_FMT "%-4s%-8s%-5s%-4s%-5s"
	ret+=websWrite(wp, "\nStations List			   \n");
	ret+=websWrite(wp, "----------------------------------------\n");
	ret+=websWrite(wp, "%-18s" WL_LOG_FMT "%-7s%-12s\n",
...
#define SHOW_STA_INFO(_p,_i,_st, _gr) {											\
		...
		ret+=websWrite(wp, "%02X:%02X:%02X:%02X:%02X:%02X " WL_LOG_FMT "%5dM %02d:%02d:%02d\n",  		\
