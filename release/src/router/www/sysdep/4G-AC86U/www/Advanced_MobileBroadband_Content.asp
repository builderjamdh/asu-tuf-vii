...
var orig_modem_autoapn = '<% nvram_get("modem_autoapn"); %>';
...
var reg_spec = '<% nvram_get("reg_spec"); %>';
...
var modem_act_band = '<% nvram_get("usb_modem_act_band"); %>';
var modem_act_scc = '<% nvram_get("usb_modem_act_scc"); %>'.replace(",", ", ");
var modem_band = modem_act_band + ((modem_act_scc != "")? (", "+ modem_act_scc) : "");
...
var modem_act_num = '<% nvram_get("usb_modem_act_num"); %>';
var orig_modem_sms_phone = parseUnicodeToChar('<% nvram_get("modem_sms_phone"); %>');
var orig_modem_bytes_data_warning = '<% nvram_get("modem_bytes_data_warning"); %>';
var default_warning_msg = "This is a alert about the data usage is over: ";
var default_limit_msg = "The data usage has reached the limit: ";
...
function initial(){
	...
	if(modem_act_num == "" && sim_state != "-1"){
		get_modem_status("number");
	}
	...
	document.form.sms_phone.value = orig_modem_sms_phone;

	show_lte_band(orig_modem_lte_band);
	change_apn_mode(orig_modem_autoapn);
	show_dateList();
...
function get_modem_status(status){
	$.ajax({
		url: "/do_modem_simstatus.cgi",
		type: "POST",
		data: {
			"status": status
		},
		error: function(){
			console.log("get_modem_status error");
		},
		success: function() {
			if(status == "number"){
				modem_act_num = httpApi.nvramGet(["usb_modem_act_num"], true).usb_modem_act_num;
				document.getElementById("modem_act_number").innerHTML = modem_act_num;
			}
		}
	});
...
function applyRule(){
	...
	if(document.form.modem_bytes_data_limit.value != modem_bytes_data_limit){
		...
		msg = default_limit_msg + document.form.data_limit.value + unit;
		document.form.modem_sms_message2.value = parseStrToUnicode(msg);
		document.form.modem_sms_message2.disabled = false;
	}

	if(document.form.modem_bytes_data_warning.value != orig_modem_bytes_data_warning){
		var unit = "";
		var msg = "";
		...
		msg = default_warning_msg + document.form.data_warning.value + unit;
		document.form.modem_sms_message1.value = parseStrToUnicode(msg);
		document.form.modem_sms_message1.disabled = false;
	}
...
*/
	...
	if(document.form.sms_phone.value != ""){
		if(!validator.isMobileNumber(document.form.sms_phone.value)){
			$("#phone_hint").html("Wrong Mobile number!");
			$("#phone_hint").show();
			document.form.sms_phone.focus();
			document.form.sms_phone.select();
			return;
		}
		else{
			document.form.modem_sms_phone.disabled = false;
			document.form.modem_sms_phone.value = parseStrToUnicode(document.form.sms_phone.value);
		}
	}
	else{
		if(document.form.modem_sms_limit.value == "1"){
			document.form.sms_phone.focus();
			document.form.sms_phone.select();
			return;
		}
	 ...
	 $.ajax({
    	...
    	success: function(response){
			...
			if(mobile_state == 2 && mobile_sbstate == 0 && mobile_auxstate != 1){
				...
				document.getElementById("mstatus").innerHTML = '<img src="/images/mConnected.png">';
				...
				if(sim_status == 2){
					...
					document.getElementById("connection_status").innerHTML = "<#Scan#>";
					document.getElementById("mconnect_status").innerHTML = "<#Scan#>";
				}
				else{
...
function simact_result_str(result_no){
	...
	switch(result_no){
		case "25":
			result_string = "<#Mobile_fail_unlock_sim#>";
			break;
		case "28":
			result_string = "<#Mobile_fail_unlock_simpin#>";
			break;
		case "30":
			result_string = "<#Mobile_sim_lock#>";
			break;
		case "34":
			result_string = "<#Mobile_fail_enable_PIN#>";
			break;
		case "35":
			result_string = "<#Mobile_fail_disable_PIN#>";
			break;
		case "38":
...
function check_sim_details(){
	...
	$.ajax({
    	...
    	success: function(response){
			...
			if(modem_act_cellid.length > 0 && modem_act_cellid != "0"){
				document.getElementById("cellid_tr").style.display = "";
				document.getElementById("modem_cellid").innerHTML = modem_act_cellid;
				show_wstatus = 1;
			}

			if(sim_operation.length > 0){
				document.getElementById("connection_type_tr").style.display = "";
				document.getElementById("connection_type").innerHTML = sim_operation;
				show_wstatus = 1;
			}

			if(modem_band.length > 0){
				document.getElementById("band_tr").style.display = "";
				document.getElementById("band").innerHTML = modem_band;
				show_wstatus = 1;
			}
			...
			if(modem_act_sinr.length > 0 && !isNaN(modem_act_sinr) && modem_act_sinr != "0"){
...
function change_apn_mode(apn_mode){
	if(apn_mode == "1"){//Automatic
		...
		if(orig_modem_autoapn == apn_mode){
			...
			document.getElementById("modem_apn_div").innerHTML = "";
			document.getElementById("modem_dialnum_div").innerHTML = "";
			document.getElementById("modem_user_div").innerHTML = "";
			document.getElementById("modem_pass_div").innerHTML = "";
		}
	}
	else{//Manual
		/*
		...
		*/
		...
		if(orig_modem_autoapn != apn_mode){
			document.form.modem_apn.value = "";
			document.form.modem_dialnum.value = "";
			document.form.modem_user.value = "";
			document.form.modem_pass.value = "";
		}
		...
		success: function(response){
			...
			if( lte_update_status == '')
				...
				if(lte_update_val == 0)
					...
					if(lte_update_val > 1)
						document.getElementById("update_msg").innerHTML = "<#Mobile_fail_upgrade_ltesw#>";
					else{
						document.getElementById("usb_modem_act_swver").innerHTML = usb_modem_act_swver;
						document.getElementById("update_msg").innerHTML = "<#Mobile_success_upgrade_ltesw#>";
					}
				}
			}
		}
...
function update_lte_fw(){
	if(sim_state != "-1"){
		alert("<#Mobile_upgrade_hint#>");
		return;
	}
	else{
		document.simact_form.action_mode.value = "update_lte_fw";
		document.simact_form.submit();
		document.getElementById("loading_block2").innerHTML = "<#Mobile_upgrading_wait#> <#Main_alert_proceeding_desc5#>";
		showLoadingBar(390);
...
function show_lte_band(band){
	var all_band = [];
	var band_3G = [];
	var ce_all_band = ["B1", "B3", "B5", "B7", "B8", "B20", "B28", "B32", "B38", "B40", "B41"];
	var ce_band_3G = ["B1", "B3", "B5", "B8"];
	var fcc_all_band = ["B2", "B4", "B5", "B9", "B12", "B13", "B14", "B17", "B25", "B26", "B30", "B40", "B41", "B66"];
	var fcc_band_3G = ["B2", "B4", "B5", "B9"];

	if(based_modelid == "4G-AX56")
		ce_all_band.splice(ce_all_band.indexOf("B32"), 1);

	if(reg_spec == "FCC"){
		all_band = fcc_all_band;
		band_3G = fcc_band_3G;
	}
	else{
		all_band = ce_all_band;
		band_3G = ce_band_3G;
	}

	$("#modem_lte_band option").remove();
	$("#modem_lte_band").append($("<option></option>")
								.attr("value", "auto")
								.text("<#Auto#>"));

	if(band == "3"){
		for(var i = 0; i < band_3G.length; i++){
			var selected = false;
			if(band  == band_3G[i])
				selected = true;

			$("#modem_lte_band").append($("<option></option>")
										.attr("value", band_3G[i])
		...
		for(var i = 0; i < all_band.length; i++){
			...
			$("#modem_lte_band").append(
									$("<option></option>")
									.attr("value", all_band[i])
									.attr("selected", selected)
									.text(all_band[i]));
		}
	}
...
function reset_modem(){
	document.simact_form.action_mode.value = "resetmodem";
	showLoadingBar(60);
	document.simact_form.submit();
}

function reboot_modem(){
	document.simact_form.action_mode.value = "rebootmodem";
	showLoadingBar(60);
	document.simact_form.submit();
}
...
<input type="hidden" name="modem_sms_phone" value="<% nvram_get("modem_sms_phone"); %>" disabled>
<input type="hidden" name="modem_sms_message1" value="" disabled>
<input type="hidden" name="modem_sms_message2" value="" disabled>
...
<div id="mobile_status"  class="contentM_qis" style="box-shadow: 3px 3px 10px #000;">
	<table class="QISform_wireless" border=0 align="center" cellpadding="5" cellspacing="5">
		<tr>
			...
			<div style="width:600px;overflow:hidden;position:relative;left:0px;top:5px;" class="splitLine"></div>
			<div style="margin-top: 15px;"><#Mobile_status_desc1#></div>
			</td>
		</tr>
		<tr>
			<td>
				...
				<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable">
					...
					<tr><th><#Phone_Number#></th><td><div id="modem_act_number"><% nvram_get("usb_modem_act_num"); %></div></td></tr>
		 		</table>
		 		</div>
	  		</td>
		</tr>
		...
		<tr>
			<td>
				...
				<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable">
					<thead>
					<tr>
						<td colspan="2"><#Mobile_Wireless_Status#></td>
					</tr>
					</thead>
					<tr id="cellid_tr" style="display:none;"><th><#Mobile_Cell_ID#></th><td><div id="modem_cellid"><% nvram_get("usb_modem_act_cellid"); %></div></td></tr>
					<tr id="connection_type_tr" style="display:none;"><th><#Connection_Type#></th><td><div id="connection_type"><% nvram_get("usb_modem_act_operation"); %></div></td></tr>
					<tr id="band_tr" style="display:none;"><th><#Mobile_Band#></th><td><div id="band"></div></td></tr>
		...
		<tr>
			<td>
				...
				<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable">
					...
					<tr><th><#PPPConnection_x_WANLink_itemname#></th><td><div style="display: flex; align-items: center;"><div id="mstatus" style="display: none;"></div><div id="mconnect_status" style="margin-left: 5px; color:#FFF;"></div></div></td></tr>
					<tr>
						<th><#Mobile_sim_provider#></th>
						<td><span id="msim_provider" style="color:#FFF;"></span></td>
					</tr>
					<tr>
						<th><#Mobile_network_provider#></th>
						<td><div style="display: flex; align-items: center;"><div id="msignalsys" style="width: 30px; height: 25px; overflow: hidden;"></div><div id="misp"></div></div></td>
					</tr>
					...
					<tr style="display: none;"><th><#Uplink_rate#></th><td><span id="upRate" style="color:#FFF;"></span></td></tr>
					<tr style="display: none;"><th><#Downlink_rate#></th><td><span id="downRate" style="color:#FFF;"></span></td></tr>
		...
		<tr>
			<td bgcolor="#4D595D" valign="top" height="680px">
				...
				<div style="margin:5px;" class="splitLine"></div>
	      		<div class="formfontdesc"><#WAN_page_desc#></div>			  
					...
					<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable"  style="margin-top:8px">
						<thead>
						<tr>
							<td colspan="2"><#Mobile_modem_info#></td>
						</tr>
						</thead>
						<!--tr><th>Modem hardware version</th><td><div id="modem_act_hwver"><% nvram_get("usb_modem_act_hwver"); %></div></td></tr-->
						<tr>
							<th><#Mobile_modem_sw_ver#></th>
							<td>
								<div style="display: flex; align-items: center; flex-wrap: wrap;">
									<div id="usb_modem_act_swver" style="color: #FFFFFF; margin-right: 10px;"><% nvram_get("usb_modem_act_swver"); %></div>
									<div>
										<span><input name="reset_modem_btn" class="button_gen" type="button" onclick="reset_modem();" value="<#Mobile_reset_modem#>"/></span>
										<span><input name="reboot_modem_btn" class="button_gen" type="button" onclick="reboot_modem();" style="margin-left:10px;" value="<#Mobile_reboot_modem#>"/></span>
									</div>
								</div>
							</td>
						</tr>
						<tr id="new_version_tr" style="display:none;"><th><#Mobile_new_sw_ver#></th><td><span id="new_version" style="color:#FFF;"></span><input id="lte_update_btn" name="lte_update_btn" class="button_gen" type="button" onclick="update_lte_fw();" style="margin-left:10px;" value="<#LANHostConfig_x_DDNSStatus_buttonname#>"/><div id="update_msg" style="color:#FC0;"></div></td></tr>
						<tr><th>IMEI</th><td><div id="modem_act_imei"><% nvram_get("usb_modem_act_imei"); %></div></td></tr>
					</table>
					<div id="lte_update_note" style="color:#FFCC00; font-size:10px; display:none;"><#Mobile_upgrade_hint#></div>
				...
				<table id="connection_table" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3"  class="FormTable" style="margin-top:8px">
				  	...
				  	<tr>
						...
						<td>
							...
							<div><img onclick="Show_status()" style="cursor:pointer;position:absolute;margin-left:440px;margin-top:-25px;" title="<#Mobile_inf#>" src="/images/info.png"></div>
						</td>		
				  	</tr>
					...
					<tr id="newtork_type_tr">
						...
						<td>
							<select name="modem_mode" id="modem_mode" class="input_option" onchange="show_lte_band(this.value);">
					...
					<tr>
						<th width="40%"><#Mobile_lte_band#></th>
						<td>
				...
				<table id="traffic_table" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3"  class="FormTable" style="margin-top:8px">
				  ...
				  <tr>
					<th><a class="hintstyle" href="javascript:void(0);" onclick="openHint(35, 1);"><#Mobile_reset_usage#></a></th>
					<td>
				  ...
				  <tr>
					<th><a class="hintstyle" href="javascript:void(0);" onclick="openHint(35, 2);"><#Mobile_usage_limit#></a></th>
					<td>
				  ...
				  <tr>
					<th><a class="hintstyle" href="javascript:void(0);" onclick="openHint(35, 3);"><#Mobile_usage_warning#></a></th>
					<td>
				  ...
				  <tr id="sms_notif_tr">
					<th width="40%"><a class="hintstyle" href="javascript:void(0);" onclick="openHint(35, 4);"><#Mobile_SMS_Notification#></a></th>
					<td>
				  ...
				  <tr id="sms_phone_tr" style="display: none">
					...
					<td>
						<div style="display: flex; align-items: center;">
							<div><input type="text" maxlength="16" class="input_15_table" name="sms_phone" value="" onblur="return check_mobile_number(this);" autocorrect="off" autocapitalize="off"/></div>
							<div id="phone_hint" style="margin-left: 5px; color: #FFCC00; display: none;"></div>
						</div>
					</td>
				  </tr>
				</table>
			  	...
			  	<table id="apn_table" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3"  class="FormTable" style="margin-top:8px">
					...
					<tr style="display: none">
          			...
          			<tr>
						...
						<td>
						<input id="modem_apn" name="modem_apn" class="input_20_table" style="margin-left:0px;" type="text" value="<% nvram_get("modem_apn"); %>" autocorrect="off" autocapitalize="off"/>
					...
					<tr>
						...
						<td>
							<input id="modem_dialnum" name="modem_dialnum" class="input_20_table" style="margin-left:0px;" type="text" value="<% nvram_get("modem_dialnum"); %>" autocorrect="off" autocapitalize="off"/>
						</td>
					</tr>
				...
				<table id="sim_mgnt_table" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3"  class="FormTable" style="margin-top:8px">
				  <thead>
				  	<tr>
						<td colspan="2"><#Mobile_sim_pin_management#></td>
				  	</tr>
				  </thead>
					...
					<tr id="pin_code_tr" style="display:none;">
						...
						<td>
							...
							<span id="save_pin_ckb_span"><input type="checkbox" name="save_pin_ckb" id="save_pin_ckb" checked><#Mobile_save_pin#></input></span>
