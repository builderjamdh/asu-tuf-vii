...
function save_applyData(wan_unit){
	var pvc_shift = "1" + load_wan_unit + "0";
	...
	*/
	...
	if(wan_bonding_support  && applyData.hasOwnProperty("bond_wan") && (orig_bond_wan != applyData["bond_wan"])){
...
function change_popup_settingsItem(wan_proto){
	...
	else if(wan_proto == "v6plus" || wan_proto == "ocnvc" || wan_proto == "dslite" || wan_proto == "v6opt"){
...
*/
	if(type == "user_defined")
		...
		if(Softwire46_support){
			...
			if(dslite_support)
				connection_type_options.push({"text":"DS-Lite","value":"dslite"});
			if(v6option_support)
				connection_type_options.push({"text":"<#IPv6_opt#>","value":"v6opt"});
		}
	}
	...
	var dhcpfilter_enabled_parm = {"title":"<#BasicConfig_EnableDHCPFilter_itemname#>", "type":"switch", "id":"wan_dhcpfilter_enable", "container_id":"dhcpfilter_enable_container"};
...
function remove_bridge(){
	...
	if(Softwire46_support){
		...
		if(v6option_support){
			var var_item8 = new Option("<#IPv6_opt#>", "v6opt");
			document.form.wan_proto.options.add(var_item8);
		}
	}
}
function add_pvc() {
...
function showMSWANList(){
	...
	if(wan_service_num == 0){
		...
		var row_count=0;
		var ms_pvc_shift = "1" + load_wan_unit + "0";
		var ms_pvc = 0;
		...
		for(var i = 0; i < MSWANList.length; i++){			
			if (MSWANList[i][0] != "0") {
				...
				else if (MSWANList[i][1]=="v6opt") cell[3].innerHTML = "<center><#IPv6_opt#></center>";
...
function disable_all_ctrl() {
	var $desc_tmp_default = (top.webWrapper)? "<div style='max-width:95%'><#dsl_wan_page_desc#></div>" : `<#dsl_wan_page_desc#>`;
	document.getElementById("desc_default").innerHTML = $desc_tmp_default;
...
function enable_all_ctrl(pvc) {
	...
	var $desc_tmp_edit = (top.webWrapper)? "<div style='max-width:95%'><#Layer3Forwarding_x_ConnectionType_sectiondesc#></div>" : `<#Layer3Forwarding_x_ConnectionType_sectiondesc#>`;
	document.getElementById("desc_edit").innerHTML = $desc_tmp_edit;
...
function change_wan_proto_type(proto_type){
	...
	if(proto_type == "pppoe" || proto_type == "pppoa"){
		...
		if((proto_type == "v6plus" && wan_proto_orig == "v6plus") || (proto_type == "ocnvc" && wan_proto_orig == "ocnvc") || (proto_type == "v6opt" && wan_proto_orig == "v6opt"))
...
function confirm_autowan_change(){
	...
	if(wan_obj.hasOwnProperty("extra_settings")){
		...
		$.each(extra_settings, function(key) {
			if(document.getElementsByName(key).length > 0){
				...
				extra_setting_list.push({ nvram: key, value: extra_settings[key] });
				$('<input>').attr({
					type: 'hidden',
					name: key,
	...
	setTimeout(function(){
		if($(".popup_edit_profile_container").is(":visible")){
			...
			if(extra_setting_list.length > 0){
				$.each(extra_setting_list, function(key) {
					let nvram_name = extra_setting_list[key].nvram;
					let nvram_value = extra_setting_list[key].value;
					applyData[nvram_name] = nvram_value;
				});
			}
			applyData["rc_service"] = "reboot";
			showLoading();
								...
								<div class="container">
									...
									<table id="t2BC" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
										...
										<tr style="display: none;">
											<th><a class="hintstyle" href="javascript:void(0);" onClick=""><#BasicConfig_EnableDHCPFilter_itemname#></a></th>
											<td>
