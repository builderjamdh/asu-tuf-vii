...
#define EEPROM_INIT_FILE "/etc/Wireless/iNIC/iNIC_e2p.bin"
	...
	char pressure_offset_buf[PRESSURE_LEN+1];
	if (FRead(&pressure_offset_buf, OFFSET_PRESSURE, PRESSURE_LEN) < 0) {
		_dprintf("READ Pressure offset: Out of scope\n");
	} else {
		if (pressure_offset_buf[0] == 0xff){
			nvram_set("TDK_Pdelta", "0");
		}else{
			nvram_set("TDK_Pdelta", pressure_offset_buf);
		}
	}
#endif
...
void reinit_hwnat(int unit)
{
	...
	default:	/* load hwnat */
		...
		case WAN_V6OPTION:
			...
			break;
		case WAN_DSLITE:
			doSystem("echo %d > /sys/kernel/debug/hnat/mape_toggle", 0);
			break;
		}
#endif
#if defined (RTCONFIG_WLMODULE_MT7615E_AP) && !defined(RTCONFIG_RALINK_MT7622)
