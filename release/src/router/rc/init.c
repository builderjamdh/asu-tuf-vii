...
#endif
#if !defined(RTBE58U) && !defined(TUFBE3600) && !defined(RTBE58U_V2) && !defined(TUFBE3600_V2) && !defined(RTBE55) && !defined(RTBE92U) && !defined(RTBE95U) && !defined(RTBE82U) && !defined(TUFBE82) && !defined(RTBE82M)  && !defined(RTBE58U_PRO) && !defined(RTBE58_GO) && !defined(GSBE18000) && !defined(GSBE12000) && !defined(GS7_PRO)
			setLANLedOn();
#endif
#ifdef RTBE86U
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
		...
		add_rc_support("tuf");
		/* --- 魔改固件: 解锁隐藏功能 start --- */
		add_rc_support("tagged_based_vlan");   // 802.1Q Tag-Based VLAN UI
		add_rc_support("mtlancfg");             // VLAN Switch/Profile 页面
		add_rc_support("vlan");                 // VLAN 后端支持
		add_rc_support("lacp");                 // Link Aggregation (LACP) UI
		add_rc_support("dfs");                  // DFS 信道支持
		add_rc_support("mumimo");               // MU-MIMO 选项
		add_rc_support("smart_connect");        // 频段导航 v1
		add_rc_support("smart_connect_v2");     // 频段导航 v2
		add_rc_support("reboot_schedule");      // 定时重启
		add_rc_support("dnsfilter");            // DNS 过滤
		add_rc_support("dnspriv");              // DNS Privacy (DoT)
		add_rc_support("traffic_analyzer");     // 流量分析
		add_rc_support("traffic_limiter");      // 流量限制
		add_rc_support("dblog");                // 诊断日志
		add_rc_support("dhdlog");               // DHD 驱动日志
		add_rc_support("email");                // 邮件通知
		add_rc_support("nt_center");            // 通知中心
		add_rc_support("fwupgrade");            // 固件升级
		add_rc_support("betaupg");              // Beta 升级
		add_rc_support("cloudcheck");           // 云检查
		add_rc_support("cloudsync");            // 云同步
		add_rc_support("findasus");             // ASUS 设备发现
		add_rc_support("letsencrypt");          // Let's Encrypt
		add_rc_support("ssh");                  // SSH
		add_rc_support("snmp");                 // SNMP
		add_rc_support("ipv6pt");               // IPv6 Pass-Through
		add_rc_support("ipv6");                 // IPv6
		add_rc_support("v6option");             // IPv6 选项
		add_rc_support("wifi6e");               // WiFi 6E (预置支持)
		/* --- 魔改固件: 解锁隐藏功能 end --- */
#endif
#ifdef RTAX58U
		if (!strncmp(nvram_safe_get("territory_code"), "CX", 2)) {
...
//		add_rc_support("WIFI_LOGO");
	...
	case MODEL_RTBE58U_V2:
		...
		if (!nvram_get("ct_max"))
			nvram_set("ct_max", "300000");
		/* --- 魔改固件: WiFi 全解锁 (TUF-BE3600 V2) --- */
		nvram_set("acs_dfs", "1");              // 启用 DFS 信道
		nvram_set("acs_band3", "1");            // 解锁 UNII-3
		nvram_set("wl0_radio_pwrsave", "0");    // 关闭 2.4G 省电
		nvram_set("wl1_radio_pwrsave", "0");    // 关闭 5G 省电
		/* 移除 ED 阈值限制 */
		nvram_unset("sb/1/eu_edthresh2g");
		nvram_unset("sb/1/eu_edthresh5g");
#ifdef RTBE55
...
//		add_rc_support("WIFI_LOGO");
		...
		add_rc_support("ofdma");
		add_rc_support("wpa3");
		/* --- 魔改固件: 解锁隐藏功能 (TUF-BE3600 V2) --- */
		add_rc_support("tagged_based_vlan");   // 802.1Q VLAN
		add_rc_support("mtlancfg");             // VLAN Switch/Profile
		add_rc_support("vlan");                 // VLAN 后端支持
		add_rc_support("lacp");                 // Link Aggregation
		add_rc_support("dfs");                  // DFS 信道
		add_rc_support("wifi7");                // WiFi 7
		add_rc_support("mumimo");               // MU-MIMO
		add_rc_support("smart_connect");        // 频段导航 v1
		add_rc_support("smart_connect_v2");     // 频段导航 v2
		add_rc_support("reboot_schedule");      // 定时重启
		add_rc_support("dnsfilter");            // DNS 过滤
		add_rc_support("dnspriv");              // DNS Privacy
		add_rc_support("traffic_analyzer");     // 流量分析
		add_rc_support("traffic_limiter");      // 流量限制
		add_rc_support("dblog");                // 诊断日志
		add_rc_support("dhdlog");               // DHD 驱动日志
		add_rc_support("email");                // 邮件通知
		add_rc_support("nt_center");            // 通知中心
		add_rc_support("fullcone");             // FullCone NAT
		add_rc_support("getrealip");            // STUN 真实 IP
		add_rc_support("cloudcheck");           // 云检查
		add_rc_support("findasus");             // ASUS 设备发现
		add_rc_support("ssh");                  // SSH
		add_rc_support("snmp");                 // SNMP
		...
		add_rc_support("ipv6pt");               // IPv6 Pass-Through
		add_rc_support("v6option");             // IPv6 选项
		/* --- 魔改固件: AP 模式管理界面开放 (TUF-BE3600 V2) --- */
		nvram_set("http_wanport", "8080");
		nvram_set("ap_mgmt_mode", "1");
		nvram_set("ap_smartdiscover", "1");
		/* --- 魔改固件: 云控默认关闭 (TUF-BE3600 V2) --- */
		nvram_set("asus_cloud_enable", "0");
		/* --- 魔改固件: BWDPI 默认禁用 (TUF-BE3600 V2) --- */
		nvram_set("disable_bwdpi", "1");
		/* --- 魔改固件: FullCone NAT 默认启用 (TUF-BE3600 V2) --- */
		nvram_set("nat_type", "1");
		/* --- 魔改固件: 管理界面 LAN+WAN 开放 (M26) --- */
		nvram_set("misc_http_x", "1");
		nvram_set("misc_httpport_x", "8080");
		nvram_set("misc_httpsport_x", "8443");
		/* --- 魔改固件: AP 模式 IP 访问 + App 发现 (M28) --- */
		nvram_set("ap_mgmt_mode", "1");
		nvram_set("ap_smartdiscover", "1");
		/* --- 魔改固件: STUN 自定义默认值 (M09) --- */
		nvram_set("stun_server", "stun.l.google.com");
		nvram_set("stun_port", "19302");
		nvram_set("stun_interval", "300");
		/* --- 魔改固件: UPnP 限速默认关闭 (M08) --- */
		nvram_set("upnp_maxrate_enable", "0");
		/* --- 魔改固件: VLAN/多WAN 默认关闭 (M29) --- */
		nvram_set("vlan_enable", "0");
		nvram_set("multiwand_enable", "0");
		/* --- 魔改固件: BBR TCP 加速 + AP 模式 WAN 硬件转发 + 防火墙 (M33/M34/M35) --- */
		if (sw_mode() == SW_MODE_AP && nvram_get_int("ap_wan_hwaccel")) {
			nvram_set("fc_disable", "0");
			nvram_set("ap_wan_fc_disable", "0");
			f_write_string("/proc/sys/net/bridge/bridge-nf-call-iptables", "1", 0, 0);
		}
		nvram_set("fb_custom_enable", "0");
		break;
#endif
#if defined(RTBE92U)
	case MODEL_RTBE92U:
...
//		nvram_set("webs_update_enable", "0");
	...
	/* --- 魔改固件: 云控总开关 - 禁用所有 ASUS 云服务 --- */
	if (cloud_disabled()) {
		logmessage("cloud_control", "ASUS cloud services disabled by user (asus_cloud_enable=0)");
		nvram_set("ddns_enable_x", "0");
		nvram_set("enable_cloudsync", "0");
		nvram_set("enable_cloudcheck", "0");
		nvram_set("fb_enable", "0");
		nvram_set("enable_webdav_lock", "0");
		nvram_set("enable_findasus", "0");
		nvram_set("wps_enable", "0");
		/* 清除注册的 DDNS 信息 */
		if (nvram_match("ddns_server_x", "WWW.ASUS.COM") ||
		    nvram_match("ddns_server_x", "WWW.ASUS.COM.CN")) {
			nvram_set("ddns_hostname_x", "");
			nvram_set("ddns_server_x", "");
		}
		nvram_commit();
...
#if defined(RTCONFIG_HND_ROUTER_BE_4916) || defined(RTCONFIG_HND_ROUTER_AX)
	if (nvram_get_int("bbr_enable")) {
		f_write_string("/proc/sys/net/core/default_qdisc", "fq", 0, 0);
		f_write_string("/proc/sys/net/ipv4/tcp_congestion_control", "bbr", 0, 0);
	}
#endif
