		...
		if ((lan_ifnames = strdup(nvram_safe_get("lan_ifnames"))) != NULL) {
			...
			while ((ifname = strsep(&p, " ")) != NULL) {
				...
				if ((nvram_match(strcat_r(prefix, "radio", tmp), "0")
					...
					ledbh_war(ifname);
#endif
					printf("%s, psta chk: wlconf/wl %s down/off.\n", __func__, ifname); fflush(stdout);
					eval("wlconf", ifname, "down");
					eval("wl", "-i", ifname, "radio", "off");
				} else{
					printf("%s, psta chk: wlconf %s start.\n", __func__, ifname); fflush(stdout);
...
static void set_ai_vlan_itf(void) {	
	_dprintf("AI_VLAN %s %d\n", __FUNCTION__, __LINE__);
	logmessage("AI_VLAN", "%s %d\n", __FUNCTION__, __LINE__);
	/* This interface is for ensuring the connection between router and aiboard */
	eval("ip", "link", "add", "link", "eth.ai", "name", "eth.ai-10", "type", "vlan", "id", "10");
	eval("ip", "addr", "add", "169.254.0.1/32", "dev", "eth.ai-10");
	eval("ip", "link", "set", "eth.ai-10", "up");
	eval("ip", "route", "add", "169.254.0.2/32", "dev", "eth.ai-10");
}
#endif
...
void start_lan(void)
{
	...
	if (nvram_match("wifison_ready", "1"))
	{
		...
		if (nvram_match("pd_low", "2")) {
			_dprintf("%s, not include wlX to lan_ifnames due pd_low(2)\n", __func__);
		...
		if ((lan_ifnames = strdup(nvram_safe_get("lan_ifnames"))) != NULL) {
			...
			while ((ifname = strsep(&p, " ")) != NULL) {
				...
				if (guest_wlif(ifname)) {
					...
					// Don't disabled bss_info if wlX.Y_bss_expire > 0 when RTCONFIG_MULTILAN_CFG is enabled. 
					// Because a new nvram wlX.Y_expiretime is introduced for access time, no need to check wlX.Y_expire.
...
/* remove disabled vap from lan_ifnames to avoid hyd stop running */
...
#endif
				{
					...
					_dprintf("reset lan port hwaddr [%s][%s]\n", ifname, get_lan_hwaddr());
					eval("ifconfig", ifname, "hw", "ether", get_lan_hwaddr());
#endif
					if (ifconfig(ifname, IFUP | IFF_ALLMULTI, NULL, NULL) != 0)
						continue;
#if defined(GTBE19000AI) || defined(GTBE96_AI)
					if(strncmp(ifname, "eth.ai", 6) == 0){
						set_ai_vlan_itf();
						_dprintf("AI_VLAN %s %d after eth.ai up\n", __FUNCTION__, __LINE__);
						logmessage("AI_VLAN", "%s %d after eth.ai up\n", __FUNCTION__, __LINE__);
					}
#endif

#if defined(RTAX55) || defined(RTAX1800) || defined(RTBE58U) || defined(TUFBE3600) || defined(RTBE58U_V2) || defined(TUFBE3600_V2) || defined(RTBE55)
...
/* compatible for old release with Lyra APP */
	...
	strlcpy(ifr.ifr_name, lan_ifname, IFNAMSIZ);
#if defined(EBG15) || defined(EBG19)
	if (ioctl(sfd, SIOCGIFHWADDR, &ifr) == 0) nvram_set("lan_hwaddr", get_lan_hwaddr());
#else
...
_dprintf("nat_rule: stop_nat_rules 1.\n");
	...
	if (sw_mode() == SW_MODE_ROUTER) {
		...
		handle_sdn_feature(ALL_SDN, SDN_FEATURE_WAN, RC_SERVICE_START);	//for wan bridge case
#endif
	}
#endif
...
#if defined(RTCONFIG_NOTIFICATION_CENTER)
static void _nc_send_usb_tethering_event()
{
...
void hotplug_net(void)
{
	...
	if (add_event) {
		...
		if (is_wds_vlan_if == 0) is_wds_vlan_if = wgn_is_wds_guest_vlan(interface);
#endif

#if defined(RTCONFIG_MULTILAN_CFG)
		if (is_wds_vlan_if == 0) is_wds_vlan_if = apg_is_wds_vlan_if(interface);
...
#if defined(RTCONFIG_MULTILAN_CFG)
		(void)apg_hotplug_net(interface, 1);
...
#if defined(RTCONFIG_MULTILAN_CFG)
		(void)apg_hotplug_net(interface, 0);
#endif	/* RTCONFIG_MULTILAN_CFG */
	...
	// Android phone, RNDIS interface, NCM, qmi_wwan.
		...
		if(!strcmp(action, "add")){
...
#endif
#ifdef RTCONFIG_MULTIWAN_IF
#ifndef RTCONFIG_MULTIWAN_PROFILE
			...
			if(nvram_match("wans_usb_bk", "1") && nvram_match("wans_usb_bk_act", "1"))
			{
				...
				char *p;
				nvram_safe_get_r("wans_mt_ioport", wans_mt_ioport, sizeof(wans_mt_ioport));
				if (strncmp(wans_mt_ioport, "usb", 3))
				{
			...
			if(nvram_match("wans_usb_bk", "1") && nvram_match("wans_usb_bk_act", "1"))
			{
				...
				if (strncmp(wans_mt_ioport, "usb", 3))
				{
					...
					if (p)
						*p = '\0';
					nvram_set("wans_mt_ioport", wans_mt_ioport);
					nvram_set("wans_mt_group", "1");
					nvram_set("wans_mt_weight", "1");
					nvram_set_int("mtwan1_enable", 0);
					mtwan_init_nvram();
					nvram_set_int("wans_usb_bk_act", 0);
				}
...
lan_up(char *lan_ifname)
{
	...
	if (repeater_mode() || psr_mode() || dpsta_mode() || dpsr_mode()) {
		...
		if ((wl_ifnames = apg_all_lan_ifnames()) != NULL) {
	...
	if(access_point_mode()
		|| ((repeater_mode()
			...
			|| dpsr_mode()
#endif
		    ) && (nvram_get_int("wlc_state") == WLC_STATE_CONNECTED || (*nvram_safe_get("lan_gateway") && nvram_match("lan_proto", "static"))))
#if defined(RTCONFIG_AMAS)
		|| (nvram_get_int("re_mode") == 1)
#endif
	) {
		_dprintf("%s, Do route add %s/%s\n", __func__, lan_ifname, nvram_safe_get("lan_gateway"));
		route_add(lan_ifname, 0, "0.0.0.0", nvram_safe_get("lan_gateway"), "0.0.0.0");

		refresh_ntpc();
	} else
		_dprintf("%s, not do route add %s/%s\n", __func__, lan_ifname, nvram_safe_get("lan_gateway"));
...
{
	char *fbwifi_argv[] = {"fb_wifi_config" ,NULL};
	pid_t pid;

	if(nvram_invmatch("fbwifi_id","off") && nvram_invmatch("fbwifi_id","") && nvram_invmatch("fbwifi_secret", ""))
		_eval(fbwifi_argv, NULL, 0, &pid);
	else{
		_dprintf("[%s][%d] need to get fbwifi_id and fbwifi_secret first!\n", __func__, __LINE__);
		return;
	}
