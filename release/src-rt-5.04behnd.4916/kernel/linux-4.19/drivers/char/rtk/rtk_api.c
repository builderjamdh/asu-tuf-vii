...
int rtk_ext_swctl_init(void)
{
    ...
    {
        rtk_port_phyAutoNegoAbility_set(4 - port, &phy_ability);
    }
...
//    portmask.bits[0]=0x1F;
...
#ifdef RTL8367S_RGMII_MODE
    mode = MODE_EXT_RGMII;
    port = EXT_PORT1;
#ifdef EBG19
    rtk_port_rgmiiDelayExt_set(port, 0, 0);
#else
    rtk_port_rgmiiDelayExt_set(port, 1, 0);
#endif
#else
    ...
    if ((ret = rtk_port_macForceLinkExt_set(port, mode, &mac_cfg))!= RT_ERR_OK)
    {
        printk("init switch failed. %s:%d---%d\n", __FUNCTION__, __LINE__, ret);
        return ret;
    }
    rtk_port_macForceLinkExt_get(port, &mode, &mac_cfg);
    printk("init switch %s:%d---port:%d,mode:%d, linkstatus:%d\n", __FUNCTION__, __LINE__, port, mode, mac_cfg.link);
#ifdef RTL8367S_RGMII_MODE
#if 0
