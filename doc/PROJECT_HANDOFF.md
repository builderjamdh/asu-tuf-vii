我将为您生成一份完整的清理版本。这份文档已修复所有乱码字符，使用标准UTF-8编码，并优化了格式。
...
### 1.1 目标
基于 ASUS 官方 GPL 源码，对 **TUF-AX3600 V2 (BCM6764)** 路由器固件进行深度魔改，解锁所有官方隐藏的高级功能，增强WiFi 7性能与CPU调度能力。
...
| M02 | 解锁 rc_support 隐藏功能 | 部分完成 | P0 | 已添加标志位到init.c，仍需适配到TUF-AX3600 V2 |
...
| M08 | UPnP 带宽限制 | 待实施 | P1 | 新增 UPnP 带宽控制 UI 和限速逻辑 |
| M09 | STUN 自定义 | 待实施 | P1 | STUN 服务器自定义、接口选择 |
| M10 | FullCone NAT 防火墙模式 | 待实施 | P1 | 启用 FullCone NAT 并作为默认选项 |
| M11 | 移除网络神盾 (AiProtection) | 待实施 | P1 | 可选移除TrendMicro bwdpi 组件 |
| M12 | UU 加速器 AP 模式 + WOL 持久化 | 待实施 | P1 | AP 模式下保持UU + WOL 服务不断 |
...
| M16 | 多WAN 管理 (内置 multiwand) | 待实施 | P2 | 多WAN 拨号 + 策略路由 |
...
| M21 | 移除 ASD / AHS 组件 | 已实施 | P1 | 移除 ASUS 诊断服务 ASD 和心跳服务AHS |
| M22 | 更新 curl 组件 | 待实施 | P1 | 升级 curl 到最新稳定版 |
| M23 | 移除 AWS IoT 智能家居 | 已实施 | P1 | 移除亚马逊AWS IoT 智能家居集成 |
| M24 | 禁用自动更新 / 更新检查 | 已实施 | P0 | 禁用固件自动更新和更新检查 |
| M25 | 优化后台字体 | 待实施 | P2 | Web 管理后台字体优化 |
| M26 | 开放管理界面(LAN+WAN) | 待实施 | P0 | 管理界面默认在LAN和WAN均开放 |
| M27 | AiMesh 节点 IP 管理 | 待实施 | P1 | AiMesh 模式下使用节点IP 访问 Web 管理 |
| M28 | AP 模式 IP 访问 + 手机 App 发现 | 待实施 | P0 | AP 模式保留 IP 访问，手机App 可局域网发现 |
| M29 | 三插件深度内核联动 | 待实施 | P0 | cpupower/multiwand/vlan 与内核底层深度联动 |
| M30 | DNSMasq + ipset + base64 + 内核模块嵌入 | 待实施 | P0 | 深度嵌入并启用dnsmasq ipset, base64 及必要内核模块 |
...
| M35 | 防火墙自定义规则管理 | 已实施 | P1 | Web 界面管理自定义iptables 规则 |
...
|------|-------------|------------|------|------|
| **router-sysdep 目录** | 92 个 | 66 个 | 减少26 个 | 空目录占位符 |
| **kernel 子目录** | linux-4.19 + **bcmkernel + dts** | 仅linux-4.19 | 缺少bcmkernel + dts | **关键缺失** |
| **hostTools 子目录** | **360 个文件** | 347 个文件 | 缺少13 个 | squashfs_4.2 等工具 |
| **prebuilt .o 文件** | 6 个(avs/dhd/emf/hnd/igs/wl) | 6 个(相同) | 一致 | 闭源预编译 |
| **bootloaders** | 一个 | 一个 | 一致 | U-Boot + ARM TF |
| **rdp** | 一个 | 一个 | 一致 | RDP 项目文件 |
...
| `bcmdrivers/broadcom/net` | 5943 | 5777 | 166 | **重要** |
| `bcmdrivers/broadcom/net/wl/impl105` | 5755 | 5591 | 164 | **关键** |
| `bcmdrivers/broadcom/net/wl/impl105/main/components/phy` | 108 | 5 | **103** | **极度关键** |
| `bcmdrivers/broadcom/net/wl/impl105/main/components/phy/ac` | 存在 | **缺失** | 全部 | **WiFi PHY AC 域** |
| `bcmdrivers/broadcom/net/wl/impl105/main/components/phy/cmn` | 存在 | **缺失** | 全部 | **WiFi PHY 公共域** |
...
| `bcmdrivers/broadcom/char/gpon` | GPON 驱动 | 低(TUF 无DSL) | 已同步(空目录) |
...
| `bcmdrivers/broadcom/net/eapfwd` | EAP 转发域 | 中 | 已同步(空目录) |
| **`bcmdrivers/broadcom/net/wl/impl105/main/components/phy/ac`** | **WiFi PHY AC 域** | **极度关键** | 空目录占位符(0 个子目录，无文件) |
| **`bcmdrivers/broadcom/net/wl/impl105/main/components/phy/cmn`** | **WiFi PHY 公共域** | **极度关键** | 空目录占位符(1 个子目录，无文件) |
| **`bcmdrivers/broadcom/net/wl/impl105/main/components/avs`** | **ASUS Wireless Security** | **极度关键** | 空目录占位符(个子目录，无文件) |
| **`bcmdrivers/broadcom/net/wl/impl105/main/components/clm-api`** | **信道列表管理** | **极度关键** | 空目录占位符(个子目录，无文件) |
| **`bcmdrivers/broadcom/net/wl/impl105/main/components/pasn`** | **认证服务** | **极度关键** | 空目录占位符(个子目录，无文件) |
| **`bcmdrivers/broadcom/net/wl/impl105/main/components/shared`** | **WiFi 共享组件** | **极度关键** | 空目录占位符(个子目录，无文件) |
...
> **重要发现**：官方GPL tarball 中phy/ac、phy/cmn、avs、clm-api、pasn、shared 等目录**都是空目录占位符**(只有子目录结构，没有.c/.h/.mk 文件)。这些文件是 ASUS 内部闭源源码的一部分，不会出现在 GPL 发布版中。
...
### 2.5 部分缺失(官方tarball 中有但GitHub 不完整)
...
**`cp: cannot stat` 的不是prebuilt 目录的文件，而是 platform.mak 中期望从编译输出目录复制的文件。**
...
**这些 `bcm_enet.o`, `wfd.o`, `bdmf.o`, `rdpa.o` 等80+ 个文件应该是从各自的源目录编译后放置到prebuilt 目录的，或者是来自ASUS 提供的闭源预编译二进制。**

### 3.4 固件"能编译但不能用"的真正原因

| 问题 | 影响 |
|------|------|
| **80+ 个Broadcom prebuilt 文件缺失** | 网卡驱动、BCM 总线管理、RDPA 硬件加速等核心功能完全缺失 |
| **Busybox setprofile/Kconfig 失败** | Busybox 工具链不完整 |
| **大量组件因`-k` 被跳过** | httpd、dropbear、各种库可能编译不完整 |
| **26 个router-sysdep 目录缺失** | MDM(设备管理)、NAS(网络存储)、hnd_dpsta(双并发 STA)等功能缺失 |
| **bcmkernel 和dts 目录缺失** | 内核模块和设备树不完整 |
...
# 2. 同步 bcmkernel 和dts(如果官方tarball 中有)
...
### 4.2 方案 B：如果官方tarball 也不完整

那说明缺失的 prebuilt .o 文件(80+ 个)是**ASUS 内部闭源二进制**，不会出现在 GPL 发布版中。这种情况下:
- 你需要从第三方团队获取完整的 prebuilt 文件集
- 或者联系ASUS 获取完整源码
- 或者尝试从其他 TUF-BE3600 V2 编译者的发布中获取

### 4.3 方案 C：修改platform.mak 跳过 prebuilt 依赖
...
**推荐方案 A**：在本地 WSL2 Ubuntu 或独立Linux 服务器上部署 Self-Hosted Runner。
      ...
      - name: Create Release
        ...
        with:
          ...
          body: |
            ...
            ### 刷入方法
            1. 登录路由器Web 管理 (192.168.50.1)
            2. 系统管理 -> 固件更新 -> 上传 .trx 文件
            3. 等待重启完成

            ### 注意事项
            - 首次刷入建议恢复出厂设置
            - 已移除ASD/AHS/FRS/AWS IoT 组件
          files: "*.trx"
          draft: true
```
...
| 步骤 | 说明 |
|------|------|
| `on.push` | 仅在 `release/src/` 和SDK 目录变更时触发，避免无关提交触发构建 |
| `workflow_dispatch` | 手动触发，配合`clean` 步骤可执行全量重新编译 |
| `timeout-minutes: 360` | HND 固件首次全量编译可能需要2-4 小时 |
| `build.log` | 编译日志完整保存，失败时上传供排查 |
| `upload-artifact` | 固件产物保留 30 天，可从 Actions 页面下载 |
| `release` job | 仅在推送`v*` tag 时自动创建GitHub Release |
...
# 下载最新编译产物
gh run download --workflow=build.yml -n TUF-AX3600_V2-*
...
GitHub-Hosted Runner 默认磁盘空间有限，需在workflow 开头清理空间：
...
将工具链压缩包上传到 GitHub Releases 或私有存储，在workflow 中动态下载：
...
- [ ] **同步官方 tarball 缺失内容到GitHub 仓库**(从官方 tarball 复制所有缺失的目录和文件)
- [ ] **验证编译**(同步后重新触发CI，确认80+ prebuilt 错误消失)
- [ ] **修复 Busybox setprofile/Kconfig 失败**(安装ncurses 或使用`make config` 替代 `make menuconfig`)
- [ ] **修复 `[: too many arguments` shell 语法错误**(检查Makefile 中的条件判断语法)
- [ ] **解锁 rc_support 隐藏功能**(适配 TUF-AX3600 V2 的rc_support 标志位)
- [ ] **开放管理界面(LAN+WAN)**(defaults.c + init.c 修改)
- [ ] **AP 模式 IP 访问 + 手机 App 发现**(AP 模式下保持Web 管理可达)
...
- [ ] **UPnP 带宽限制**(miniupnpd 扩展 + iptables tc 联动)
- [ ] **STUN 自定义**(ministun 调用参数改为 NVRAM 读取)
- [ ] **移除 AiProtection / bwdpi**(编译时移除 TrendMicro 组件)
- [ ] **更新 curl 组件**(升级到最新稳定版)
- [ ] **三插件深度内核联动**(cpupower/multiwand/vlan 与内核底层深度联动)

### 6.3 低优先级(P2/P3)

- [ ] **DFS 信道指定套件**
- [ ] **CPU 频率管理套件 (内置 cpupower)**
- [ ] **多WAN 管理 (内置 multiwand)**
- [ ] **VLAN 管理套件 (内置 vlan)**
- [ ] **增强错误日志系统**
- [ ] **主题系统**
- [ ] **Pro 高级功能选项**
...
| 组件 | 路径 (相对于release/src-rt-5.04behnd.4916/) |
|------|---------|
| 构建目标定义 | `target.mak` (release/src-rt/ 中, 150 行) |
| 芯片配置 | `chip_profile.mak` 第58 行`TUF-AX3600_V2_CHIP_PROFILE=6764` |
| 内核配置 | `targets/96764GW/96764GW.TUF-AX3600_V2` |
| NVRAM 默认 | `targets/nvram/6764/TUF-AX3600_V2.nvm` |
| 设备树 | `kernel/dts/6764/6764.dtsi` (SoC 线) |
...
| 模型初始化 | `release/src/router/rc/init.c` (行20054-20192 行) |
| LED 温度传感器DTS | `kernel/dts/6764/6764.dtsi` 中`therm0: brcm-therm` |
...
首次编译可能需要2-4 小时(取决于机器性能)。后续增量编译10-30 分钟。
...
├── TUF-AX3600_V2_xxx.trx          # 主固件(通过 Web UI 刷入)
└── TUF-AX3600_V2_xxx_cferom.bin   # 带CFE 的完整镜像(救援模式用)
```
...
1. 登录路由器Web 管理界面 (通常 `192.168.50.1`)
2. 进入「系统管理」→「固件更新」
3. 上传编译好的 `.trx` 文件
4. 等待刷入完成并自动重启

**救援模式刷入**：按住Reset 按钮通电，电源灯闪烁时通过 ASUS Firmware Restoration 工具上传固件。
...
-> 工具链未安装或PATH 未设置。检查`/opt/toolchains/` 目录。
...
-> 检查CFE/NVRAM 兼容性。可通过串口 TTL 查看启动日志。

### WSL2 编译慢

-> 确保源码放在 WSL2 原生 ext4 文件系统中(不在/mnt/c/)，否则 I/O 性能极差。
...
│   │   └── router/
│   │       ├── rc/               # 系统初始化与服务管理
│   │       ├── httpd/            # Web 服务端(处理 ASP/CGI)
│   │       ├── shared/           # 共享库与预编译对象
│   │       ├── www/              # Web 前端 (ASP/JS/CSS/图片)
│   │       └── [258 个组件目录]
│   ├── src-rt/                   # 旧式 MIPS 平台 (本项目不使用)
│   ├── src-rt-5.04behnd.4916/    # Broadcom HND SDK (本项目核心平台)
...
当前 TUF-AX3600 V2 的rc_support = `mssid 2.4G 5G update usbX1 switchctrl manual_stb 11AX pwrctrl nandflash movistarTriple wifi2017 app ofdma wpa3`
...
这三个插件原为koolshare 软件中心格式，将被深度内置整合进固件：

| 插件 | 功能 | 语言 | 改造要求 |
|------|------|------|----------|
| **cpupower** | CPU 调度控制、频率管理、温度监控 | Shell + ASP | dbus→NVRAM, RC 服务化，EJ 路由 |
| **multiwand** | 多WAN 管理 (PPPoE/DHCP/静态IP/策略路由) | Shell + ASP | dbus→NVRAM, 集成到rc/services.c 和firewall.c |
| **vlan** | VLAN 管理 (802.1Q/端口绑定/端口聚合) | Shell + ASP | dbus→NVRAM, 集成到init-broadcom.c |
...
| 2026-07-18 | M02.0 | rc_support 解锁 (TUF-BE3600 V2) |
...
| 2026-07-18 | M08/M09 | UPnP 限制+ STUN 自定义 |
...
| 2026-07-29 | CI-03 | 串行重建 httpd/dropbear 时显式传递CC 交叉编译链 |
| 2026-07-29 | CI-04 | 添加 httpd 二进制存在性验证，失败时CI 终止 |
...
### 分析对象
`C:\Users\Dahi\Downloads\logs_82329543767.zip` (最近一次CI 构建日志)

### 根本原因

**问题 1：CONFIG_SITE 环境变量在"Compile (two-phase)"步骤中丢失**

GitHub Actions 中每个`run:` 步骤是独立shell，"Build Firmware" 步骤设置的`CONFIG_SITE=/tmp/config.site` 不会传递到"Compile (two-phase)" 步骤。
...
**问题 3：串行重建(Serial rebuild)使用主机gcc 而非交叉编译链**

```bash
# Compile 步骤中串行重建httpd 时
make -C "$SRCDIR/router" -j1 -k httpd CONFIGURE="$CONFIGURE_OVERRIDE"
```

`CONFIGURE_OVERRIDE` 不包含`CC=`，configure 找不到`arm-buildroot-linux-gnueabi-gcc` 则fallback 到`/usr/bin/gcc`(主机gcc)，导致:
```
...
| 3 | 串行重建传递CC 参数 | `.github/workflows/build.yml` | `make ... httpd CC=...` 显式传递交叉编译器 |
...
1. **同步官方 tarball 缺失内容**(执行sync.ps1 脚本或手动复制)
2. **验证编译**(重新触发CI，确认CONFIG_SITE 修复生效)
...
1. 生成 PROJECT_HANDOFF.md(覆盖AI_HANDOFF.md 全部内容)
2. 从官方tarball 同步 16 个缺失目录到 GitHub 仓库
3. 提交更改(commit 9be894559)

### 关键发现

**官方 GPL tarball 中phy/ac、phy/cmn、avs、clm-api、pasn、shared 等目录都是空目录占位符！**

这意味着:
- GPL tarball 中只有目录结构(50+ 个子目录)，没有 .c/.h/.mk 文件
- 这些文件是ASUS 内部闭源源码的一部分，不会出现在 GPL tarball 中
- 第三方团队能编译出可用固件，是因为他们拥有完整的内部源码(包含所有预编译 .o 文件和闭源源码)

### 下一步行动

1. **获取 ASUS 内部完整源码**(从第三方团队或 ASUS)
2. **或修改platform.mak 跳过 prebuilt 依赖**(方案C)
3. **或尝试从其他 TUF-BE3600 V2 编译者的发布中获取**
...
| ARM TF 工具链 | `/opt/toolchains/crosstools-arm-gcc-10.3-linux-4.19-glibc-2.32-binutils-2.36.1` | ARM Trusted Firmware(安全bootloader) |
